...
def build_input_image_dataset(dataset_path: FilepathType) -> Dataset:
...
def build_output_figure_dataset(dataset_path: FilepathType) -> Dataset:
    img_shape = (600, 600, 3)

    return (
        Dataset.build(dataset_path)
        .add_item_attr(name="name", shape=None, dtype="str")
        .add_item_attr(name="data", shape=img_shape, dtype=np.uint8)
        .add_item_attr(name="label", shape=None, dtype=int)
        .add_item_attr(name="prediction", shape=None, dtype=int)
        .conclude_build()
    )
...
def fig2arr(fig: Figure) -> np.ndarray:
    canvas = fig.canvas
    canvas.draw()
    arr = np.frombuffer(canvas.tostring_rgb(), dtype=np.uint8)
    arr = arr.reshape(canvas.get_width_height()[::-1] + (3,))
    return arr
...
def get_or_create_dataset(
    ...
    dataset = build_input_image_dataset(
